...
const MoviePopup = () => {
    ...
    const getMovie = async () => {
      try {
        ...
        const findIndexOfTrailer = extractMovieDetails?.data?.videos?.results?.length ?
          extractMovieDetails?.data?.videos?.results?.findIndex((item: {
            type: string
          }) => item.type === "Trailer") : -1
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent
        className={"max-w-6xl max-h-[90vh] overflow-y-auto !scrollbar-thin !scrollbar-track-transparent !scrollbar-thumb-red-600"}
      >
        {isLoading ? (
          <>
